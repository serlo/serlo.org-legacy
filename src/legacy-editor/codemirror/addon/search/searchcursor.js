      ...
      if (target.length == 1) {
        ...
        this.matches = function(reverse, pos) {
          ...
          for (;;) {
            ...
            if (
              reverse
                ? offsetB != line.length - match.length
                : offsetB != match.length
            ) {
              return
            }
            var start = Pos(pos.line, offsetA),
              end = Pos(ln, offsetB)
    ...
    find: function(reverse) {
      ...
      for (;;) {
        if ((this.pos = this.matches(reverse, pos))) {
          if (!this.pos.from || !this.pos.to) {
            console.log(this.matches, this.pos)
          }
