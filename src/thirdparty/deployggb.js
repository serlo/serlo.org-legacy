...
window.GGBApplet = function() {
  ...
  var doSetJavaCodebase = function(codebase, offline) {
    ...
    if (jnlpBaseDir === null) {
      ...
      applet.setJNLPFile(
        dir + codebase + '/' + buildJNLPFileName(isJavaOffline)
      )
    } else {
      applet.setJNLPFile(
        jnlpBaseDir +
          javaCodebaseVersion +
          '/' +
          buildJNLPFileName(isJavaOffline)
      )
    }
  }
  ...
  applet.isJavaInstalled = function() {
    if (typeof deployJava === 'undefined') {
      ...
      return (
        ...
        deployJava.versionCheck('1.4') ||
        deployJava.versionCheck('1.5.0*')
      )
    }
  }
  ...
  var fetchParametersFromTube = function(successCallback) {
    ...
    if (parameters.tubeurl !== undefined) {
      ...
      window.location.host.indexOf('geogebratube.org') > -1 ||
      ...
      window.location.host.indexOf('geomatech-test.geogebra.org') > -1
    ) {
    ...
    if (xhr.setRequestHeader) {
      // IE9's XDomainRequest does not support this method
  ...
  var JavaVersion = function() {
    ...
    for (var i = 0, size = navigator.mimeTypes.length; i < size; i++) {
      ...
      // version to use.
      if (
        (resutl = navigator.mimeTypes[i].type.match(
  ...
  var injectJavaApplet = function(appletElem, parameters) {
    ...
    if (!isJavaOffline) {
      appendParam(
        applet,
        'image',
        'http://tube.geogebra.org/images/java_loading.gif'
      )
    }
    ...
    log(
      "GeoGebra Java applet injected. Used JNLP file = '" +
        jnlpFilePath +
        "'" +
        (isOverriddenJavaCodebase
          ? " with overridden codebase '" + javaCodebase + "'."
          : '.'),
      parameters
    )
  }
  ...
  var injectHTML5Applet = function(appletElem, parameters, noPreview) {
    ...
    if (parameters.width !== undefined) {
      if (parseFloat(html5CodebaseVersion) <= 4.4) {
        ...
        if (
          parseFloat(html5CodebaseVersion) >= 5.0 &&
    ...
    if (
      ...
      parseFloat(html5CodebaseVersion) >= 4.4 &&
    ...
    if (loadScript) {
      ...
      log(
        "GeoGebra HTML5 applet injected. Codebase = '" + html5Codebase + "'.",
        parameters
      )
      appletElem.appendChild(script)
    } else {
  ...
  var injectCompiledApplet = function(appletElem, parameters, noPreview) {
    ...
    var scriptFile =
      ...
      '/applet.js' +
      (preCompiledScriptVersion != null && preCompiledScriptVersion != undefined
  ...
  var detectAppletType = function(preferredType) {
    ...
    if (preferredType === 'preferjava') {
      ...
      // type=auto
      if (applet.isJavaInstalled() && !applet.isHTML5Installed()) {
        return 'java'
      } else {
        return 'html5'
      }
  ...
  var setDefaultHTML5CodebaseForVersion = function(version, offline) {
    ...
    for (var key in modules) {
      if (
        html5CodebaseVersion.slice(modules[key].length * -1) === modules[key] ||
  ...
  var setHTML5CodebaseInternal = function(codebase, offline) {
    ...
    if (folders.length > 1) {
      if (!offline && folders[folders.length - 2] === 'webSimple') {
        // Currently we don't use webSimple for offline worksheets
        html5CodebaseScript = 'webSimple.nocache.js'
        html5CodebaseIsWebSimple = true
      } else if (
        folders[folders.length - 2] === 'web3d' ||
        folders[folders.length - 2] === 'tablet' ||
        folders[folders.length - 2] === 'phone'
      ) {
  ...
  var setDefaultJavaCodebaseForVersion = function(version) {
    ...
    var codebase =
      protocol + '//jars.geogebra.org/webstart/' + javaCodebaseVersion + '/'
  ...
  function continueInit() {
    ...
    if (html5OverwrittenCodebaseVersion !== null) {
      ...
      if (parseFloat(html5Version) < 5.0) {
        // Use 5.0 as default for html5. Change the version number here, when a new stable version is released.
        html5Version = '5.0'
      }
    }
