...
utils_1.createMigration(exports, {
    up: (db) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        yield removeSubject(db, {
            instance: 1,
            name: 'Betriebswirtschaftslehre mit Rechnungswesen',
        });
        yield removeSubject(db, { instance: 1, name: 'Deutsch als Fremdsprache' });
        yield removeSubject(db, { instance: 1, name: 'Musik' });
        yield createSubject(db, { instance: 1, name: 'Lerntipps' });
    }),
    down: (db) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        yield removeSubject(db, { instance: 1, name: 'Lerntipps' });
        yield createSubject(db, { instance: 1, name: 'Musik' });
        yield createSubject(db, { instance: 1, name: 'Deutsch als Fremdsprache' });
        yield createSubject(db, {
            instance: 1,
            name: 'Betriebswirtschaftslehre mit Rechnungswesen',
        });
    }),
...
function createSubject(db, { instance, name }) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const subjectTaxonomy = (yield db.runSql(`SELECT taxonomy.id FROM taxonomy JOIN type ON taxonomy.type_id = type.id WHERE type.name="subject" AND taxonomy.instance_id = ?`, instance))[0].id;
        yield db.runSql(`INSERT INTO uuid(trashed, discriminator) VALUES (0, "taxonomyTerm")`);
        const taxonomyTerm = yield getLastInsertedId(db);
        yield db.runSql(`INSERT INTO term(instance_id, name) VALUES (?, ?)`, instance, name);
        const term = yield getLastInsertedId(db);
        yield db.runSql(`INSERT INTO term_taxonomy(id, taxonomy_id, term_id, parent_id) VALUES (?, ?, ?, ?);`, taxonomyTerm, subjectTaxonomy, term, subjectTaxonomy);
    });
}
...
function getLastInsertedId(db) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        return (yield db.runSql(`SELECT LAST_INSERT_ID()`))[0]['LAST_INSERT_ID()'];
    });
}
