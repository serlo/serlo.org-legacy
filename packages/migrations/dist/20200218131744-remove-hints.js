...
utils_1.createMigration(exports, {
    up: (db) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        const results = yield db.runSql(`SELECT id FROM type WHERE name = 'text-hint'`);
        if (results.length !== 1) {
            throw new Error('Expected type `text-hint` to exist');
        }
        const { id } = results[0];
        yield db.runSql(`DELETE FROM uuid WHERE id = ANY (SELECT id FROM entity WHERE type_id = ?)`, id);
    }),
});
