...
var _headManager = _interopRequireDefault(require("next/dist/client/head-manager"));
...
var _pageLoader = _interopRequireDefault(require("next/dist/client/page-loader"));

var envConfig = _interopRequireWildcard(require("next/dist/next-server/lib/runtime-config"));
...
var data = JSON.parse(document.getElementById('__NEXT_DATA__' + (window.NEXT_ROOTS ? window.NEXT_ROOTS[0] : '')).textContent); // Reason:
  ...
  _createClass(Container, [{
    ...
    value: function componentDidMount() {
      ...
      if (data.nextExport && ((0, _isDynamic.isDynamicRoute)(router.pathname) || location.search || data.skeleton)) {
        ...
        router.replace(router.pathname + '?' + (0, _querystring.stringify)({ ...router.query,
...
var _default = async function _default() {
  ...
  try {
    ...
    if (!window.NEXT_ROOTS) {
      ...
      try {
        for (var _iterator = window.NEXT_ROOTS[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var id = _step.value;
          var indexOfHashtag = id.indexOf('#');
          var pageId = id.substring(0, indexOfHashtag);
          var component = await pageLoader.loadPage(pageId);
  ...
  if (!window.NEXT_ROOTS) {
    ...
    for (var _page in components) {
      var pageProps = JSON.parse(document.getElementById('__NEXT_DATA__' + _page).textContent).props;
      var _renderCtx = {
        App: App,
        page: _page,
