...
async function run() {
  try {
    const name = 'serlo-org-frontend-assets'

    const { version } = await fetchPackageJSON()

    await mkDir(path.join(distPath, '_next', 'static'), { recursive: true })
    await cp(path.join(root, 'public'), path.join(distPath))
    await cp(
      path.join(root, '.next', 'static'),
      path.join(distPath, '_next', 'static')
    )
