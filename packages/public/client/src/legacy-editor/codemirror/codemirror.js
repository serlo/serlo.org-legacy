...
window.CodeMirror = (function () {
  ...
  var ie_lt8 =
    ie && (document.documentMode == null || document.documentMode < 8)
  var ie_lt9 =
  ...
  var flipCtrlCmd =
    mac &&
    (qtwebkit || (opera && (opera_version == null || opera_version < 12.11)))
  ...
  function makeDisplay(place, docStart) {
    ...
    d.scrollbarH = elt(
      'div',
      [elt('div', null, null, 'height: 1px')],
      'CodeMirror-hscrollbar'
    )
    d.scrollbarV = elt(
      'div',
      [elt('div', null, null, 'width: 1px')],
    ...
    d.lineSpace = elt(
      'div',
      [d.measure, d.selectionDiv, d.lineDiv, d.cursor, d.otherCursor],
      null,
    ...
    d.heightForcer = elt(
      ...
      null,
      null,
      'position: absolute; height: ' + scrollerCutOff + 'px; width: 1px;'
    )
  ...
  function updateScrollbars(cm) {
    ...
    d.gutters.style.height =
      Math.max(totalHeight, d.scroller.clientHeight - scrollerCutOff) + 'px'
    ...
    if (needsH) {
      ...
      d.scrollbarH.firstChild.style.width =
        d.scroller.scrollWidth -
        d.scroller.clientWidth +
        d.scrollbarH.clientWidth +
        'px'
    } else {
  ...
  function maybeUpdateLineNumberWidth(cm) {
    ...
    if (last.length != display.lineNumChars) {
      ...
      display.lineNumInnerWidth = Math.max(
        innerW,
  ...
  function updateDisplay(cm, changes, viewPort, forced) {
    ...
    for (var first = true; ; first = false) {
      ...
      if (
        visible.from >= cm.display.showingFrom &&
        visible.to <= cm.display.showingTo
      ) {
        break
      }
    }
  ...
  function updateDisplayInner(cm, changes, visible, forced) {
    ...
    if (
      !forced &&
      changes.length == 0 &&
      visible.from > display.showingFrom &&
      visible.to < display.showingTo
    ) {
      return
    }
    ...
    var gutterW = (display.sizer.style.marginLeft =
    ...
    if (intactLines < (to - from) * 0.7) display.lineDiv.style.display = 'none'
  ...
  function updateHeightsInViewport(cm) {
    ...
    for (
      var node = display.lineDiv.firstChild, height;
      node;
        ...
        if (line.widgets && cur && cur.previousSibling) {
          for (var i = 0; i < line.widgets.length; ++i) {
            ...
            if (w.showIfHidden) {
              ...
              var wnode = prev.appendChild(
                elt('div', [w.node], 'CodeMirror-linewidget')
              )
  ...
  function buildLineElement(cm, line, lineNo, dims, reuse) {
    ...
    var bgClass = built.bgClass
      ? built.bgClass + ' ' + (line.bgClass || '')
      : line.bgClass
    if (
    ...
    if (bgClass) {
      wrap.insertBefore(
        elt('div', null, bgClass + ' CodeMirror-linebackground'),
        wrap.firstChild
      )
    }
    if (cm.options.lineNumbers || markers) {
      var gutterWrap = wrap.insertBefore(
        elt(
          'div',
          null,
          null,
      ...
      if (
        ...
        wrap.lineNumber = gutterWrap.appendChild(
          elt(
            'div',
            lineNumberFor(cm.options, lineNo),
            'CodeMirror-linenumber CodeMirror-gutter-elt',
            'left: ' +
    ...
    if (line.widgets && wrap != reuse) {
      for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
        ...
        if (widget.above) {
          wrap.insertBefore(
            node,
            cm.options.lineNumbers && line.height != 0
              ? gutterWrap
              : lineElement
          )
  ...
  function updateSelection(cm) {
    ...
    if (!collapsed) updateSelectionRange(cm)
  ...
  function updateSelectionCursor(cm) {
    ...
    if (pos.other) {
      ...
      display.otherCursor.style.height =
  ...
  function updateSelectionRange(cm) {
      ...
      fragment.appendChild(
        elt(
          ...
          'position: absolute; left: ' +
            ...
            top +
            'px; width: ' +
            (width == null ? clientWidth - left : width) +
            'px; height: ' +
            (bottom - top) +
            'px'
        )
    ...
    if (sel.from.line == sel.to.line) {
      ...
      var leftEnd = drawForLine(
        sel.from.line,
        sel.from.ch,
        singleVLine ? fromLine.text.length : null
      ).end
        ...
        if (doc.frontier >= cm.display.showingFrom) {
          ...
          if (ischange) {
            if (prevChange && prevChange.end == doc.frontier) prevChange.end++
              ...
              changed.push(
                (prevChange = { start: doc.frontier, end: doc.frontier + 1 })
              )
            }
          }
          ...
          line.stateAfter =
            doc.frontier % 5 == 0 ? copyState(doc.mode, state) : null
        }
        ++doc.frontier
        if (+new Date() > end) {
  ...
  function getStateBefore(cm, n, precise) {
    ...
    doc.iter(pos, n, function (line) {
      ...
      var save =
        pos == n - 1 ||
        pos % 5 == 0 ||
        (pos >= display.showingFrom && pos < display.showingTo)
  ...
  function findCachedMeasurement(cm, line) {
    ...
    for (var i = 0; i < cache.length; ++i) {
      ...
      if (
        memo.text == line.text &&
        memo.markedSpans == line.markedSpans &&
  ...
  function charCoords(cm, pos, context, lineObj, bias) {
    ...
    return intoCoordSystem(
      cm,
      lineObj,
      measureChar(cm, lineObj, pos.ch, null, bias),
      context
    )
  }
  ...
  function endOperation(cm) {
    ...
    if (display.maxLineChanged && !cm.options.lineWrapping && display.maxLine) {
      ...
      display.sizer.style.minWidth =
        Math.max(0, width + 3 + scrollerCutOff) + 'px'
      display.maxLineChanged = false
      var maxScrollLeft = Math.max(
        0,
        display.sizer.offsetLeft +
          display.sizer.offsetWidth -
          display.scroller.clientWidth
      )
      if (maxScrollLeft < doc.scrollLeft && !op.updateScrollPos) {
        setScrollLeft(
          cm,
          Math.min(display.scroller.scrollLeft, maxScrollLeft),
          true
        )
      }
    }
    ...
    if (
      op.changes.length ||
      op.forceUpdate ||
      (newScrollPos && newScrollPos.scrollTop != null)
    ) {
    ...
    if (op.updateScrollPos) {
      ...
      if (op.scrollToPos) {
        scrollPosIntoView(
          cm,
          clipPos(cm.doc, op.scrollToPos.from),
          clipPos(cm.doc, op.scrollToPos.to),
  ...
  function posFromMouse(cm, e, liberal) {
    ...
    if (!liberal) {
      ...
      if (
        target == display.scrollbarH ||
        target == display.scrollbarH.firstChild ||
        target == display.scrollbarV ||
        target == display.scrollbarV.firstChild ||
        target == display.scrollbarFiller ||
        target == display.gutterFiller
      ) {
        return null
      }
    }
  ...
  function onDrop(e) {
    ...
    if (files && files.length && window.FileReader && window.File) {
      ...
      if (
        ...
        !(posLess(pos, cm.doc.sel.from) || posLess(cm.doc.sel.to, pos))
      ) {
  ...
  function onKeyPress(e) {
    ...
    if (
      ((opera && (!e.which || e.which < 10)) || khtml) &&
  ...
  function onContextMenu(cm, e) {
    ...
    display.input.style.cssText =
      ...
      'px; left: ' +
      (e.clientX - 5) +
      'px; z-index: 1000; background: white; outline: none;' +
  ...
  function clipPostChange(doc, change, pos) {
    ...
    return clipToLen(
      pos,
      change.text[inside].length + (inside ? 0 : change.from.ch)
    )
  }
    ...
    if (split) {
      ...
      if (split.length) {
        makeChangeNoReadonly(
          doc,
          { from: split[0].from, to: split[0].to, text: change.text },
          selUpdate
        )
      }
    } else {
  ...
  function makeChangeSingleDocInEditor(cm, change, spans, selAfter) {
    ...
    if (
      !posLess(doc.sel.head, change.from) &&
  ...
  function addToScrollPos(cm, left, top) {
    ...
    pos.scrollTop = Math.max(
      0,
      Math.min(scroll.scrollHeight - scroll.clientHeight, pos.scrollTop + top)
    )
    pos.scrollLeft = Math.max(
      0,
      Math.min(scroll.scrollWidth - scroll.clientWidth, pos.scrollLeft + left)
    )
  }
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    var curSpaceString = line.text.match(/^\s*/)[0],
      indentation
    if (how == 'smart') {
      indentation = cm.doc.mode.indent(
        state,
    ...
    if (indentString != curSpaceString) {
      ...
      doc.sel.head.line == n &&
      doc.sel.head.ch < curSpaceString.length
    ) {
      setSelection(
        doc,
        Pos(n, curSpaceString.length),
  ...
  function findPosV(cm, pos, dir, unit) {
    ...
    if (unit == 'page') {
      ...
      y =
        pos.top +
        dir * (pageSize - (dir < 0 ? 1.5 : 0.5) * textHeight(cm.display))
    } else if (unit == 'line') {
  ...
  CodeMirror.prototype = {
    ...
    removeKeyMap: function (map) {
      ...
      for (var i = 0; i < maps.length; ++i) {
        if (
          maps[i] == map ||
          (typeof maps[i] !== 'string' && maps[i].name == map)
        ) {
          maps.splice(i, 1)
          return true
        }
    ...
    moveH: operation(null, function (dir, unit) {
      ...
      } else pos = dir < 0 ? sel.from : sel.to
    ...
    deleteH: operation(null, function (dir, unit) {
      ...
      if (!posEq(sel.from, sel.to)) {
        ...
        replaceRange(
          ...
          '',
          sel.from,
          findPosH(this.doc, sel.head, dir, unit, false),
          '+delete'
        )
      }
    ...
    scrollIntoView: operation(null, function (range, margin) {
      ...
      var sPos = calculateScrollPos(
        this,
        Math.min(coords.from.left, coords.to.left),
  ...
  TextMarker.prototype.changed = function () {
    ...
    if (pos.line >= cm.display.showingFrom && pos.line < cm.display.showingTo) {
      for (
        var node = cm.display.lineDiv.firstChild;
        node;
        node = node.nextSibling
      ) {
  ...
  function markText(doc, from, to, options, type) {
    ...
    if (marker.addToHistory) {
      addToHistory(
        ...
        { head: doc.sel.head, anchor: doc.sel.anchor },
        NaN
      )
    }
  ...
  function markedSpansBefore(old, startCh, isInsert) {
    if (old) {
      for (var i = 0, nw; i < old.length; ++i) {
        ...
        if (
          startsBefore ||
          (((marker.inclusiveLeft && marker.inclusiveRight) ||
  ...
  function stretchSpansOverChange(doc, change) {
    ...
    if (sameLine && first) {
      ...
      for (var i = 0; i < first.length; ++i) {
        if (
          first[i].from != null &&
          first[i].from == first[i].to &&
          first[i].marker.type != 'bookmark'
        ) {
          first.splice(i--, 1)
        }
      }
  ...
  function removeReadOnlyRanges(doc, from, to) {
    ...
    for (var i = 0; i < markers.length; ++i) {
      ...
      for (var j = 0; j < parts.length; ++j) {
        ...
        var newParts = [j, 1]
        if (
          posLess(p.from, m.from) ||
          (!mk.inclusiveLeft && posEq(p.from, m.from))
        ) {
  ...
  LineWidget.prototype.clear = widgetOperation(function () {
    ...
    updateLineHeight(
      this.line,
      Math.max(0, this.line.height - widgetHeight(this))
    )
  ...
  function runMode(cm, lineNo, text, mode, state, f) {
    ...
    var curStart = 0,
      curStyle = null
    var stream = new StringStream(text, cm.options.tabSize, lineNo),
      style
  ...
  function getLineStyles(cm, line) {
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
      line.styles = highlightLine(
        cm,
        line,
        (line.stateAfter = getStateBefore(cm, lineNo(line)))
      )
    }
    return line.styles
  }
  ...
  function interpretTokenStyle(style, builder) {
    ...
    for (;;) {
      ...
      if (!lineClass) break
      style =
        style.slice(0, lineClass.index) +
        style.slice(lineClass.index + lineClass[0].length)
      var prop = lineClass[1] ? 'bgClass' : 'textClass'
      if (builder[prop] == null) builder[prop] = lineClass[2]
      else if (
        !new RegExp('(?:^|s)' + lineClass[2] + '(?:$|s)').test(builder[prop])
      ) {
  ...
  function buildLineContent(cm, realLine, measure, copyWidgets) {
    ...
    if (measure && !builder.measuredSomething && !measure[0]) {
      measure[0] = builder.pre.appendChild(
        empty ? elt('span', '\u00a0') : zeroWidthElement(cm.display.measure)
      )
    }
      ...
      if (last.from + 1 == last.to && prev && last.level < prev.level) {
        ...
        if (span) {
          span.parentNode.insertBefore(
            (span.measureRight = zeroWidthElement(cm.display.measure)),
            span.nextSibling
          )
        }
      }
    ...
    var textClass = builder.textClass
      ? builder.textClass + ' ' + (realLine.textClass || '')
      : realLine.textClass
  ...
  function buildTokenMeasure(builder, text, style, startStyle, endStyle) {
    ...
    for (var i = 0; i < text.length; ++i) {
      ...
      var span = (builder.measure[builder.pos] = buildToken(
        builder,
        ch,
        style,
        start && startStyle,
        i == text.length - 1 && endStyle
      ))
    ...
    if (widget) {
      ...
      if (builder.measure) {
        if (size) {
          ...
          } else if (builder.measure[builder.pos]) return
          else {
    ...
    for (;;) {
      if (nextChange == pos) {
        ...
        if (collapsed && (collapsed.from || 0) == pos) {
          buildCollapsedSpan(
            builder,
            (collapsed.to == null ? len : collapsed.to) - pos,
            collapsed.marker,
            collapsed.from == null
          )
  ...
  function zeroWidthElement(measure) {
    if (zwspSupported == null) {
      ...
      if (measure.firstChild.offsetHeight != 0) {
        zwspSupported =
          test.offsetWidth <= 1 && test.offsetHeight > 2 && !ie_lt8
      }
    }
  ...
  var bidiOrdering = (function () {
    ...
    function charType(code) {
      ...
      } else if (code >= 0x700 && code <= 0x8ac) return 'r'
      else return 'L'
    }
    ...
    return function (str) {
      ...
      for (var i = 0; i < len; ++i) {
        ...
        if (type == ',') types[i] = 'N'
          ...
          var replace =
            (i && types[i - 1] == '!') || (end < len - 1 && types[end] == '1')
              ? '1'
              : 'N'
