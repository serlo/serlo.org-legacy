...
class NavigationProvider implements PageProviderInterface
{
    ...
    public function getTerm()
    {
        if (!is_object($this->term)) {
            $instance = $this->getInstanceManager()->findInstanceByName(
            ...
            $this->term = $this->getTaxonomyManager()->findTermByName(
                $taxonomy,
                (array) $parent['slug']
            );
        }
    ...
    public function provide(array $options)
    {
        ...
        $this->options['types'] = ArrayUtils::merge(
            $this->options['types'],
            $this->options['hidden']
        );
    ...
    protected function iterTerms($terms, $depth)
    {
        ...
        foreach ($terms as $term) {
            if (!$term->isTrashed()) {
                ...
                $current['params'] = [
                    $this->options['identifier'] => (string) $term->getId(),
                ];
                $current['label'] = $term->getName();
                $current['elements'] = $term->countAssociations(
                    null,
                    $this->filter
                );
                $children = $term->findChildrenByTaxonomyNames(
                    $this->options['types']
                );
                if (
                    in_array(
                        $term->getType()->getName(),
                        $this->options['hidden']
                    )
                ) {
