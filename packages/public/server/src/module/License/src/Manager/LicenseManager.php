...
{
    use ClassResolverAwareTrait, InstanceAwareObjectManagerAwareTrait;
    ...
    public function findAllLicenses()
    {
        $className = $this->getClassResolver()->resolveClassName(
            'License\Entity\LicenseInterface'
        );
        $licenses = $this->getObjectManager()
    ...
    public function getDefaultLicense()
    {
        ...
        if (!is_object($license)) {
            throw new Exception\Runtimeexception(
                sprintf(
                    'No default license set for for %s',
                    $license->getName()
                )
            );
        }
    ...
    public function getLicense($id, $instanceAware = true)
    {
        $className = $this->getClassResolver()->resolveClassName(
            'License\Entity\LicenseInterface'
        );
        $license = $this->getObjectManager()->find(
            $className,
            $id,
            $instanceAware
        );
        ...
        if (!is_object($license)) {
            throw new Exception\LicenseNotFoundException(
                sprintf('License not found by id `%s`.', $id)
            );
        }
    ...
    public function getLicenseForm($id = null)
    {
        if ($id !== null) {
            ...
            $license = $this->getClassResolver()->resolve(
                'License\Entity\LicenseInterface'
            );
        }
    ...
    public function injectLicense(
        ...
        $newlyCreated = !$object->getId();
        if ($newlyCreated) {
            $this->getObjectManager()->flush($object);
        }
        $this->getEventManager()->trigger('inject', $this, [
            'object' => $object,
            'license' => $license,
