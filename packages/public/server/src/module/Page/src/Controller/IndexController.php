...
class IndexController extends AbstractAPIAwareActionController
{
    ...
    public function createRevisionAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                $this->getPageManager()->flush();
                $redirectUrl = $this->plugin('url')->fromRoute('page/view', [
                    'page' => $page->getId(),
                ]);
    ...
    public function viewAction()
    {
        ...
        $revision = $pageRepository->hasCurrentRevision()
            ? $pageRepository->getCurrentRevision()
            : null;
        if (!$revision) {
            return $this->notFound();
        }
        ...
        $view = new ViewModel([
            'revision' => $revision,
            'page' => $pageRepository,
            'convert' => $this->params('convert', false),
        ]);
