    ...
    \Zend\EventManager\EventManagerAwareInterface
{
    ...
    public function findEventIds(array $options, int $limit = 100)
    {
        ...
        if (array_key_exists('userId', $options)) {
            $filter[] = 'actor_id = ' . $options['userId'];
        }
        if (array_key_exists('entityId', $options)) {
            $filter[] = 'uuid_id = ' . $options['entityId'];
        }
        ...
        $sqlIds =
            ...
            'LIMIT ' .
            $limit;
        $q = $this->objectManager->getConnection()->prepare($sqlIds);
        $q->execute();
        ...
        $sqlMeta =
            ...
            'ORDER BY id';
        $q = $this->objectManager->getConnection()->prepare($sqlMeta);
        $q->execute();
        $meta = $q->fetchAll()[0];
