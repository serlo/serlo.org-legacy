...
class AliasManager implements AliasManagerInterface
{
    ...
    public function autoAlias(
        ...
        if (!array_key_exists($name, $this->getOptions()->getAliases())) {
            throw new Exception\RuntimeException(
                sprintf('No configuration found for "%s"', $name)
            );
        }
        ...
        $alias = $this->getTokenizer()->transliterate(
            $provider,
            $object,
            $tokenString
        );
        $aliasFallback = $this->getTokenizer()->transliterate(
            $provider,
            $object,
            $fallbackString
        );
    ...
    public function createAlias(
        ...
        $class = $this->getClassResolver()->resolve(
