...
!(function() {
  ...
  var d3_select = function(s, n) {
    ...
    d3_selectMatches = function(n, s) {
      var d3_selectMatcher =
        n.matches || n[d3_vendorSymbol(n, 'matchesSelector')]
      d3_selectMatches = function(n, s) {
  ...
  d3_selectionPrototype.selectAll = function(selector) {
    ...
    for (var j = -1, m = this.length; ++j < m; ) {
      for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
        if ((node = group[i])) {
          subgroups.push(
            (subgroup = d3_array(selector.call(node, node.__data__, i, j)))
          )
  ...
  d3_selectionPrototype.insert = function(name, before) {
    ...
    return this.select(function() {
      return this.insertBefore(
        ...
        before.apply(this, arguments) || null
      )
    })
  }
  ...
  function d3_selection_onFilter(listener, argumentz) {
    ...
    return function(e) {
      ...
      if (
        !related ||
        (related !== target && !(related.compareDocumentPosition(target) & 8))
      ) {
        l.call(target, e)
      }
    }
      ...
      return function() {
        var that = this,
          ...
          dragSubject = d3
            ...
            .on(move + dragName, moved)
            .on(end + dragName, ended),
  ...
  d3.behavior.zoom = function() {
        ...
        if (touches.length === 1) {
          if (now - touchtime < 500) {
            ...
            zoomTo(
              that,
              p,
              locations0[p.identifier],
              Math.floor(Math.log(view.k) / Math.LN2) + 1
            )
            d3_eventPreventDefault()
          }
          touchtime = now
    ...
    function dblclicked() {
      ...
      zoomTo(
        ...
        p,
        location(p),
        d3.event.shiftKey ? Math.ceil(k) - 1 : Math.floor(k) + 1
      )
    }
  ...
  function d3_hsl(h, s, l) {
    return this instanceof d3_hsl
      ? void ((this.h = +h), (this.s = +s), (this.l = +l))
      : arguments.length < 2
      ? h instanceof d3_hsl
        ? new d3_hsl(h.h, h.s, h.l)
  ...
  function d3_hcl(h, c, l) {
    return this instanceof d3_hcl
      ? void ((this.h = +h), (this.c = +c), (this.l = +l))
      : arguments.length < 2
      ? h instanceof d3_hcl
        ? new d3_hcl(h.h, h.c, h.l)
        : h instanceof d3_lab
  ...
  function d3_lab(l, a, b) {
    return this instanceof d3_lab
      ? void ((this.l = +l), (this.a = +a), (this.b = +b))
      : arguments.length < 2
      ? l instanceof d3_lab
        ? new d3_lab(l.l, l.a, l.b)
        : l instanceof d3_hcl
        ? d3_hcl_lab(l.h, l.c, l.l)
  ...
  function d3_rgb(r, g, b) {
    return this instanceof d3_rgb
      ? void ((this.r = ~~r), (this.g = ~~g), (this.b = ~~b))
      : arguments.length < 2
      ? r instanceof d3_rgb
        ? new d3_rgb(r.r, r.g, r.b)
  ...
  function d3_xhr(url, mimeType, response, callback) {
    ...
    function respond() {
      var status = request.status,
        result
      if (
        (!status && d3_xhrHasResponse(request)) ||
  ...
  d3.dsv = function(delimiter, mimeType) {
    ...
    function dsv(url, row, callback) {
      ...
      var xhr = d3_xhr(
        url,
        mimeType,
        row == null ? response : typedResponse(row),
        callback
      )
      xhr.row = function(_) {
        return arguments.length
    ...
    dsv.format = function(rows) {
      ...
      return [fields.map(formatValue).join(delimiter)]
        .concat(
  ...
  function d3_locale_numberFormat(locale) {
    var locale_decimal = locale.decimal,
      ...
      formatGroup =
        locale_grouping && locale_thousands
    ...
    return function(specifier) {
      ...
      return function(value) {
        ...
        var length =
          ...
          padding =
            length < width
              ? new Array((length = width - length + 1)).join(fill)
              : ''
        if (zcomma)
          before = formatGroup(
            padding + before,
            padding.length ? width - after.length : Infinity
          )
        negative += prefix
        value = before + after
        return (
  ...
  var d3_format_types = d3.map({
    ...
    r: function(x, p) {
      return (x = d3.round(x, d3_format_precision(x, p))).toFixed(
        Math.max(0, Math.min(20, d3_format_precision(x * (1 + 1e-15), p)))
      )
    }
  })
  ...
  ;[
    ...
    i = 7 - i
    var interval = (d3_time[day] = d3_time_interval(
      function(date) {
        ;(date = d3_time.day(date)).setDate(
      ...
      function(date) {
        ...
        return (
          Math.floor((d3_time.dayOfYear(date) + ((day + i) % 7)) / 7) -
          (day !== i)
        )
      }
    ))
  ...
  function d3_locale_timeFormat(locale) {
    ...
    function d3_time_parse(date, template, string, j) {
      ...
      while (i < n) {
        ...
        if (c === 37) {
          t = template.charAt(i++)
          p =
            d3_time_parsers[t in d3_time_formatPads ? template.charAt(i++) : t]
    ...
    function d3_time_parseWeekdayAbbrev(date, string, i) {
      ...
      return n
        ? ((date.w = d3_time_dayAbbrevLookup.get(n[0].toLowerCase())),
          i + n[0].length)
        : -1
    }
    function d3_time_parseWeekday(date, string, i) {
      ...
      return n
        ? ((date.w = d3_time_dayLookup.get(n[0].toLowerCase())),
          i + n[0].length)
        : -1
    }
    ...
    return function(feature) {
      ...
      if (n) {
        ...
        var best = -Infinity,
          dλ
        for (
          var n = merged.length - 1, i = 0, a = merged[n], b;
          i <= n;
          a = b, ++i
        ) {
          b = merged[i]
  ...
  function d3_geo_clipSort(a, b) {
    return (
      ((a = a.x)[0] < 0 ? a[1] - halfπ - ε : halfπ - a[1]) -
  ...
  function d3_geo_clipAntimeridianIntersect(λ0, φ0, λ1, φ1) {
    ...
    return abs(sinλ0_λ1) > ε
      ? Math.atan(
          (Math.sin(φ0) * (cosφ1 = Math.cos(φ1)) * Math.sin(λ1) -
  ...
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      while (true) {
        ...
        var λ = point[0],
          ...
          antimeridian = adλ > π,
          k = sinφ0 * sinφ
        d3_geo_areaRingSum.add(
          Math.atan2(k * sdλ * Math.sin(adλ), cosφ0 * cosφ + k * Math.cos(adλ))
        )
        polarAngle += antimeridian ? dλ + sdλ * τ : dλ
        if (antimeridian ^ (λ0 >= meridian) ^ (λ >= meridian)) {
          var arc = d3_geo_cartesianCross(
            d3_geo_cartesian(point0),
          ...
          var φarc =
            (antimeridian ^ (dλ >= 0) ? -1 : 1) * d3_asin(intersection[2])
    ...
    return (
      (polarAngle < -ε || (polarAngle < ε && d3_geo_areaRingSum < 0)) ^
      (winding & 1)
    )
  }
  function d3_geo_clipCircle(radius) {
    ...
    return d3_geo_clip(
      ...
      clipLine,
      interpolate,
      smallRadius ? [0, -radius] : [-π, radius - π]
    )
      ...
      return {
        ...
        point: function(λ, φ) {
          ...
          if (v !== v0) {
            ...
            if (
              d3_geo_sphericalEqual(point0, point2) ||
  ...
  function d3_geo_clipExtent(x0, y0, x1, y1) {
    return function(listener) {
      ...
      function insidePolygon(p) {
        ...
        for (var i = 0; i < n; ++i) {
          for (
            var j = 1, v = polygon[i], m = v.length, a = v[0], b;
            j < m;
            ++j
          ) {
            b = v[j]
  ...
  d3.geo.path = function() {
    ...
    path.projection = function(_) {
      ...
      projectStream = (projection = _)
        ? _.stream || d3_geo_pathProjectStream(_)
        : d3_identity
      return reset()
    }
  ...
  function d3_geo_projectionMutator(projectAt) {
    ...
    projection.stream = function(output) {
      ...
      stream = d3_geo_projectionRadians(
        preclip(rotate, projectResample(postclip(output)))
      )
      stream.valid = true
      return stream
    }
    projection.clipAngle = function(_) {
      ...
      preclip =
        _ == null
          ? ((clipAngle = _), d3_geo_clipAntimeridian)
    ...
    function reset() {
      projectRotate = d3_geo_compose(
        (rotate = d3_geo_rotation(δλ, δφ, δγ)),
        project
      )
  ...
  d3.geo.rotation = function(rotate) {
    rotate = d3_geo_rotation(
      (rotate[0] % 360) * d3_radians,
      rotate[1] * d3_radians,
      rotate.length > 2 ? rotate[2] * d3_radians : 0
    )
    function forward(coordinates) {
      coordinates = rotate(
        coordinates[0] * d3_radians,
  ...
  function d3_geo_rotation(δλ, δφ, δγ) {
    return δλ
      ? δφ || δγ
        ? d3_geo_compose(d3_geo_rotationλ(δλ), d3_geo_rotationφγ(δφ, δγ))
        : d3_geo_rotationλ(δλ)
      : δφ || δγ
      ? d3_geo_rotationφγ(δφ, δγ)
  ...
  d3.geo.circle = function() {
    ...
    function circle() {
      var center =
          typeof origin === 'function' ? origin.apply(this, arguments) : origin,
        rotate = d3_geo_rotation(
          -center[0] * d3_radians,
          -center[1] * d3_radians,
          0
  ...
  d3.geo.distance = function(a, b) {
    ...
    return Math.atan2(
      Math.sqrt(
        (t = cosφ1 * sinΔλ) * t +
          (t = cosφ0 * sinφ1 - sinφ0 * cosφ1 * cosΔλ) * t
      ),
  ...
  function d3_geo_interpolate(x0, y0, x1, y1) {
    var cy0 = Math.cos(y0),
      ...
      d =
        2 *
        Math.asin(
          Math.sqrt(d3_haversin(y1 - y0) + cy0 * cy1 * d3_haversin(x1 - x0))
        ),
      k = 1 / Math.sin(d)
    var interpolate = d
      ? function(t) {
  ...
  function d3_geo_lengthLineStart() {
    ...
    function nextPoint(λ, φ) {
      var sinφ = Math.sin((φ *= d3_radians)),
        ...
        cosΔλ = Math.cos(t)
      d3_geo_lengthSum += Math.atan2(
        Math.sqrt(
          (t = cosφ * Math.sin(t)) * t +
            (t = cosφ0 * sinφ - sinφ0 * cosφ * cosΔλ) * t
        ),
  ...
  function d3_geo_conicConformal(φ0, φ1) {
    var cosφ0 = Math.cos(φ0),
      ...
      n =
        φ0 === φ1
          ? Math.sin(φ0)
          : Math.log(cosφ0 / Math.cos(φ1)) / Math.log(t(φ1) / t(φ0)),
  ...
  function d3_geom_hullUpper(points) {
    ...
    for (var i = 2; i < n; i++) {
      while (
        hs > 1 &&
        d3_cross2d(points[hull[hs - 2]], points[hull[hs - 1]], points[i]) <= 0
      )
        --hs
      hull[hs++] = i
    }
  ...
  function d3_geom_voronoiClipEdges(extent) {
    ...
    while (i--) {
      e = edges[i]
      if (
        !d3_geom_voronoiConnectEdge(e, extent) ||
        !clip(e) ||
  ...
  function d3_geom_voronoiCreateEdge(lSite, rSite, va, vb) {
    ...
    d3_geom_voronoiCells[lSite.i].edges.push(
      new d3_geom_voronoiHalfEdge(edge, lSite, rSite)
    )
    d3_geom_voronoiCells[rSite.i].edges.push(
      new d3_geom_voronoiHalfEdge(edge, rSite, lSite)
    )
    return edge
  }
  ...
  function d3_geom_voronoiHalfEdge(edge, lSite, rSite) {
    ...
    this.site = lSite
    this.angle = rSite
      ? Math.atan2(rSite.y - lSite.y, rSite.x - lSite.x)
      : edge.l === lSite
  ...
  d3.geom.voronoi = function(points) {
    ...
    function voronoi(data) {
      ...
      d3_geom_voronoi(sites(data), clipExtent).cells.forEach(function(cell, i) {
        var edges = cell.edges,
          site = cell.site,
          polygon = (polygons[i] = edges.length
    ...
    voronoi.links = function(data) {
      return d3_geom_voronoi(sites(data))
    ...
    voronoi.size = function(_) {
      if (!arguments.length)
        return clipExtent === d3_geom_voronoiClipExtent
          ? null
  ...
  function d3_interpolateString(a, b) {
    ...
    ;(a = a + ''), (b = b + '')
    while (
      (am = d3_interpolate_numberA.exec(a)) &&
  ...
  function d3_transform(m) {
    ...
    this.rotate =
      (kx ? Math.atan2(r0[1], r0[0]) : Math.atan2(-r1[0], r1[1])) * d3_degrees
  ...
  d3.layout.hierarchy = function() {
    ...
    function hierarchy(root) {
      ...
      while ((node = stack.pop()) != null) {
        nodes.push(node)
        if (
          (childs = children.call(hierarchy, node, node.depth)) &&
          (n = childs.length)
        ) {
          var n, childs, child
          while (--n >= 0) {
  ...
  d3.layout.pack = function() {
    ...
    function pack(d, i) {
      ...
      if (padding) {
        var dr =
          (padding *
            (radius ? 1 : Math.max((2 * root.r) / w, (2 * root.r) / h))) /
          2
        d3_layout_hierarchyVisitAfter(root, function(d) {
          d.r += dr
        })
      ...
      d3_layout_packTransform(
        ...
        w / 2,
        h / 2,
        radius ? 1 : 1 / Math.max((2 * root.r) / w, (2 * root.r) / h)
      )
      return nodes
    }
  ...
  d3.layout.tree = function() {
    ...
    function wrapTree(root0) {
      ...
      while ((node1 = queue.pop()) != null) {
        for (
          var children = node1.children, child, i = 0, n = children.length;
          i < n;
          ++i
        ) {
          queue.push(
    ...
    function apportion(v, w, ancestor) {
      if (w) {
        ...
        while (
          ...
          if (shift > 0) {
            d3_layout_treeMove(
              d3_layout_treeAncestor(vim, v, ancestor),
              v,
              shift
            )
            sip += shift
  ...
  function d3_scale_linearNice(domain, m) {
    return d3_scale_nice(
      domain,
      d3_scale_niceStep(d3_scale_linearTickRange(domain, m)[2])
    )
  }
  ...
  function d3_scale_linearFormatPrecision(type, range) {
    ...
    return type in d3_scale_linearFormatSignificant
      ? Math.abs(
          p - d3_scale_linearPrecision(Math.max(abs(range[0]), abs(range[1])))
        ) + +(type !== 'e')
      : p - (type === '%') * 2
  }
  ...
  function d3_scale_log(linear, base, positive, domain) {
    function log(x) {
      return (
        (positive ? Math.log(x < 0 ? 0 : x) : -Math.log(x > 0 ? 0 : -x)) /
        Math.log(base)
      )
    }
    function pow(x) {
    ...
    scale.nice = function() {
      var niced = d3_scale_nice(
        domain.map(log),
        positive ? Math : d3_scale_logNiceNegative
      )
      linear.domain(niced)
  ...
  function d3_scale_ordinal(domain, ranger) {
    ...
    function scale(x) {
      return range[
        ((index.get(x) ||
          (ranger.t === 'range' ? index.set(x, domain.push(x)) : NaN)) -
          1) %
          range.length
      ]
    }
      ...
      var start = x[0],
        ...
        step =
          domain.length < 2
            ...
            : ((stop - start) / (domain.length - 1 + padding)) | 0
      range = steps(
        start +
          Math.round(
            (step * padding) / 2 +
              (stop - start - (domain.length - 1 + padding) * step) / 2
          ),
        step
      )
      rangeBand = 0
    ...
    scale.rangeRoundBands = function(x, padding, outerPadding) {
      ...
      range = steps(
        start +
          Math.round((stop - start - (domain.length - padding) * step) / 2),
        step
      )
      if (reverse) range.reverse()
  ...
  d3.svg.arc = function() {
    ...
    function arc() {
      ...
      if (da >= τε)
        return (
          circleSegment(r1, cw) + (r0 ? circleSegment(r0, 1 - cw) : '') + 'Z'
        )
      var rc,
        cr,
        rp,
        ...
        if (x1 != null) {
          ...
          if (rc === rc1) {
            path.push(
              ...
              ' 0 ',
              (1 - cw) ^
                d3_svg_arcSweep(t30[1][0], t30[1][1], t12[1][0], t12[1][1]),
              ',',
              cw,
              ' ',
              t12[1],
        ...
        if (x3 != null) {
          ...
          if (rc === rc0) {
            path.push(
              ...
              r0,
              ' 0 ',
              cw ^ d3_svg_arcSweep(t21[1][0], t21[1][1], t03[1][0], t03[1][1]),
              ',',
              1 - cw,
              ' ',
              t03[1],
  ...
  function d3_selection_interruptNS(ns) {
    return function() {
      ...
      if ((lock = this[ns]) && (active = lock[lock.active])) {
        ...
        lock.active += 0.5
        active.event &&
          active.event.interrupt.call(this, this.__data__, active.index)
      }
    }
  }
  ...
  function d3_transition_tween(groups, name, value, tween) {
    ...
    return d3_selection_each(
      ...
      typeof value === 'function'
        ? function(node, i, j) {
            node[ns][id].tween.set(
              name,
              tween(value.call(node, node.__data__, i, j))
            )
          }
        : ((value = tween(value)),
          function(node) {
  ...
  d3_transitionPrototype.attr = function(nameNS, value) {
    ...
    var interpolate =
        nameNS == 'transform' ? d3_interpolateTransform : d3_interpolate,
    ...
    return d3_transition_tween(
      this,
      'attr.' + nameNS,
      value,
      name.local ? attrTweenNS : attrTween
    )
  }
  ...
  d3_transitionPrototype.style = function(name, value, priority) {
    ...
    function styleString(b) {
      return b == null
        ...
        : ((b += ''),
          function() {
            var a = d3_window(this)
                .getComputedStyle(this, null)
                .getPropertyValue(name),
              i
            return (
              a !== b &&
              ((i = d3_interpolate(a, b)),
              function(t) {
  ...
  d3_transitionPrototype.duration = function(value) {
    ...
    return d3_selection_each(
      ...
      typeof value === 'function'
        ? function(node, i, j) {
            node[ns][id].duration = Math.max(
              1,
              value.call(node, node.__data__, i, j)
            )
          }
  ...
  d3_transitionPrototype.each = function(type, listener) {
    ...
    if (arguments.length < 2) {
      ...
      d3_selection_each(this, function(node) {
        ...
        ;(
          transition.event ||
          (transition.event = d3.dispatch('start', 'end', 'interrupt'))
        ).on(type, listener)
      })
    }
    return this
  ...
  d3.svg.brush = function() {
    ...
    function brushstart() {
      ...
      function move1(point, scale, i) {
        ...
        ? yClamp
        : xClamp)
          ? Math.max(r0, Math.min(r1, point[i]))
          : point[i]
        if (dragging) {
    ...
    brush.empty = function() {
      return (
        (!!x && xExtent[0] == xExtent[1]) || (!!y && yExtent[0] == yExtent[1])
      )
    }
  ...
  function d3_time_scale(linear, methods, format) {
    ...
    function tickMethod(extent, count) {
      ...
      return i == d3_time_scaleSteps.length
        ...
        : methods[
            target / d3_time_scaleSteps[i - 1] < d3_time_scaleSteps[i] / target
              ? i - 1
              : i
          ]
    }
    scale.nice = function(interval, skip) {
      ...
      function skipped(date) {
        return (
          !isNaN(date) &&
          !interval.range(date, d3_time_scaleDate(+date + 1), skip).length
        )
      }
      return scale.domain(
        d3_scale_nice(
  ...
  var d3_time_scaleMilliseconds = {
    range: function(start, stop, step) {
      return d3
        .range(Math.ceil(start / step) * step, +stop, step)
        .map(d3_time_scaleDate)
    },
    floor: d3_identity,
    ceil: d3_identity
  }
